...
class BiasFinder:
  ...
  def __init__(self, config: dict, root: str = ".", wandb=None, model="bert-base-uncased"):
    ...
    # Limit sequence length to avoid positional embedding overflow (reserve room for special tokens)
    tok = AutoTokenizer.from_pretrained(model)
    self.max_seq_len = min(
      config.get("max_length", tok.model_max_length - 2),
      tok.model_max_length - 2
    )
    self.max_seq_len = max(self.max_seq_len, 8)  # keep a sensible floor
    # Ensure pipeline tokenizer respects the shorter max length
    tok.model_max_length = self.max_seq_len
    tok.truncation_side = "right"

    # Prefer GPU if available; pipeline accepts device index (cpu=-1)
    self.device = 0 if torch.cuda.is_available() else -1

    self.mask_filler = pipeline(
      "fill-mask", model=model, tokenizer=tok, device=self.device
    )
  ...
  def scan(self, text):
    ...
    # Truncate overly long sequences and rebuild a text string (keep [MASK], drop other specials)
    tokenized = self.mask_filler.tokenizer(
      text,
      truncation=True,
    ...
    tokens = self.mask_filler.tokenizer.convert_ids_to_tokens(tokenized["input_ids"])
    specials = set(self.mask_filler.tokenizer.all_special_tokens)
    mask_token = self.mask_filler.tokenizer.mask_token
    tokens = [t for t in tokens if (t not in specials) or (t == mask_token)]
    truncated_text = self.mask_filler.tokenizer.convert_tokens_to_string(tokens)

    predictions = self.mask_filler(truncated_text)
    score = 0.0
  ...
  def modify_data(self, dataset, subset=None):
    ...
    # Sort by highest-bias rows first to align with "most confident" selection downstream
