...
class DataTransformer:
  ...
  def masking_actions(self, text, word_type=None, method=None):
    ...
    if word_type=="female_name":
      female_list = list(self.female_names)
      if not female_list:
        return gender_word
      opp_gender_word = female_list[self.random.randint(0, len(female_list) - 1)]
    
    elif word_type=="male_name":
      male_list = list(self.male_names)
      if not male_list:
        return gender_word
      opp_gender_word = male_list[self.random.randint(0, len(male_list) - 1)]
    ...
    if method=="naive_masking": #mask with config["naive_mask_token"]
      ...
      "smart_fixed_masking",
      "smart_random_masking",
      "female_first_smart_random_masking",
      "female_first_smart_fixed_masking",
    ]:
      # ACL'23 reproduction: fixed phrase template "[1] or [2]" with deterministic ordering
      phrase = "[1] or [2]"

      # default: keep detected word first
      first, second = gender_word, opp_gender_word

      # ensure female-first variants place female term first
      if method in ["female_first_smart_random_masking", "female_first_smart_fixed_masking"]:
        if word_type == "female_name" or word_type == "female_nonname":
          first, second = gender_word, opp_gender_word
        else:
          first, second = opp_gender_word, gender_word
